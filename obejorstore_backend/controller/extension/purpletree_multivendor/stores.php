...
class ControllerExtensionPurpletreeMultivendorStores extends Controller{
	...
	public function edit(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			//$this->request->post['allow_category'] = implode(',',$this->request->post['allow_category']);
			
			//$this->model_extension_purpletree_multivendor_stores->assignCategories($seller_id, $this->request->post);			
			...
			$cus_seller_email = $this->model_extension_purpletree_multivendor_stores->getCustomerEmailId($sellers_id);
			if((isset($this->request->post['seller_transaction'])&& $this->request->post['seller_transaction']!='') && (isset($this->request->post['seller_amount'])&& $this->request->post['seller_amount']!='')){
...
/* 		if (isset($this->request->post['type'])) {
		...
		}elseif(isset($categoryinfo['type'])) {
		...
		} elseif(isset($categoryinfo['category_id'])) {
			$categoryinfo['category_id'] = explode(',',$categoryinfo['category_id']);
			$data['allow_category'] = $categoryinfo['category_id'];
			$data['allow_category1'] = $categoryinfo['category_id'];
		} else {
